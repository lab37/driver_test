...
Page({
  ...
  onLoad:function(args){//传过来的参数：{model:c1,subject:1,testType:order}
...
var _fn = {
  ...
  showExplain:function(questionId){//显示本题解释
    var curQuestion =  test.getCurrentQuestion().question;
    curQuestion.showExplains = true;
  },
  lockQuestion:function(questionId){//锁住当前题
    var curQuestion = test.getCurrentQuestion().question;
    curQuestion.lockQuestion = true;
  },
  getPageData:function(){//决定显示哪个page
  ...
  getExamIndex:function(){//获取所有考题
    return us.last(getCurrentPages()).data.examDataIndex;
  }

};
var record = {//定义record对象，记录当前考题，记录对错。
